...
| Second `ENVIRONMENT` / `DEV_OPERATOR_EMAIL` | Sourced from `.dev.vars.example`; leave blank or remove. The flow stores this `ENVIRONMENT` as a remote secret either way, so the first build warns that the deploy replaces it with the config value `production`; expected, and gone on later builds |
...
1. On a domain controller or management host with RSAT, run
   `Export-CheckGpoConfig.ps1` (downloadable from the wizard's import
   panel, or `scripts/` in the repo). It asks for the GPO's name, reads
   the Check policy values (Chrome hive, Edge fallback), and prints
   managed-storage-shaped JSON, also saved to `check-gpo-export.json`.
   Read-only; the GPO is never modified.
...
- **Branding**: a tenant branding field left blank inherits the default; any
  ...
  default logo and color** on its Branding tab: no custom logo is served,
  the primary color is pinned to Check's default (#F77F00), and the
  extension shows Check's built-in look. The tenant's stored color is kept
  but not applied until the opt-out is lifted. The same choice exists at
