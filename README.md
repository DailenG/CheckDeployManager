...
| ![First-login setup wizard with live step state](docs/screenshots/setup-wizard.png) | ![Tenant onboarding wizard ending at a per-method deployment checklist](docs/screenshots/tenant-onboard.png) |
| ![Tenant defaults editor: branding, default logo, policy defaults](docs/screenshots/settings-tenant-defaults.png) | ![Upstream sync status and snapshot history](docs/screenshots/upstream.png) |
| ![Audit log with per-action detail](docs/screenshots/audit.png) | |
...
On your first login the dashboard opens a **setup wizard** that walks steps 5 through 7 in-app (instance settings, first upstream sync, tenant zero) and verifies each against live server state. When adding clients later, the **Onboard wizard** on the tenant list walks each tenant from creation through branding, policy, publish, a per-method deployment checklist, and first-fetch verification. The list below stays as the manual reference; both wizards can be skipped and never trap you.
